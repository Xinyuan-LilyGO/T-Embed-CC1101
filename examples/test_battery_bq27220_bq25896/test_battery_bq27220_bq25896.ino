...
bool verifyGaugeChargeParameters()
{
    BQ27220DMData chargeConfig[] = {
        ...
        makeDmU16Entry(BQ27220DMAddressConfigurationCurrentThresholdsQuitCurrent,
                       kBqQuitCurrentMa),
        makeDmEndEntry(),
    };
...
bool initGauge()
{
    ...
    Serial.println(readGaugeControlWord(Control_HW_VERSION), HEX);
    ...
    Serial.print(F(" mV / "));
    Serial.print(gauge.getChargeCurrent());
    Serial.println(F(" mA"));
...
bool configurePmuChargePath(const bool resetRegisters)
{
    ...
    pmu.setBatteryRechargeThresholdOffset(
        kRechargeThresholdOffsetMv >= 200 ?
        XPowersPPM::RECHARGE_OFFSET_200MV :
    ...
    pmu.enableCharge();
    if (!pmu.isEnableCharge()) {
        Serial.println(F("[BAT] Failed to enable charge path."));
        return false;
    }
    ...
    if (staticConfigMismatch) {
        ...
        Serial.print(F("[BAT] Actual target/pre/fast/term/rechg/timer/hiz: "));
        ...
        Serial.print(F("/"));
        Serial.println(pmu.isHizMode() ? F("yes") : F("no"));
        return false;
    }

    // DPDM/input source detection can legally rewrite IINLIM after init, so
    // we arm it only after static charge parameters have been verified.
    armUsbSourceDetection();
    Serial.print(F("[BAT] Initial IINLIM before DPDM: "));
    Serial.print(pmu.getInputCurrentLimit());
    Serial.println(F(" mA"));

    Serial.print(F("[BAT] BQ25896 target/pre/fast/term/rechg: "));
    Serial.print(kChargeTargetVoltageMv);
    Serial.print(F("mV / "));
...
bool refreshMetrics()
{
    ...
    if (!rawBusHasPower &&
        next.vbusPresent &&
        isUsableInputBusStatus(latchedInputBusStatus)) {
...
void printHelp()
{
    ...
    Serial.println(F("  charge on  - enable BQ25896 charging"));
    Serial.println(F("  charge off - disable BQ25896 charging"));
    Serial.println(F("  shutdown   - power off via BQ25896 (battery-only)"));
    Serial.println();
}
...
void printStatus()
{
    ...
    Serial.println(F(" mV"));
    Serial.print(F("[BAT] PMU VBAT:        "));
    Serial.print(metrics.pmuBattVoltageMv);
    Serial.println(F(" mV"));
    ...
    Serial.print(F("[BAT] Power Good/HIZ:  "));
    Serial.print(metrics.powerGood ? F("yes") : F("no"));
    Serial.print(F(" / "));
    Serial.println(metrics.hizMode ? F("yes") : F("no"));
...
void handleCommand(const String& line)
{
    ...
    if (!pmuReady || !gaugeReady) {
        Serial.println(F("[BAT] Battery example not ready."));
        return;
    }
...
void setup()
{
    ...
    Serial.println();
    Serial.println(F("T-Embed BQ27220 + BQ25896 battery test"));

    pinMode(BOARD_PWR_EN, OUTPUT);
    digitalWrite(BOARD_PWR_EN, HIGH);

    userButton.pin = BOARD_USER_KEY;
    pinMode(userButton.pin, INPUT_PULLUP);
    ...
    gaugeReady = initGauge();
    if (!gaugeReady) {
        showFatalError(F("[BAT] BQ27220 init/config failed."));
    }

    pmuReady = configurePmu();
    if (!pmuReady) {
        showFatalError(F("[BAT] BQ25896 config failed."));
    }
